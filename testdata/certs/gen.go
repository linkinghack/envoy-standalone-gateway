...
//
// 产物：一个自签测试 CA（ca.crt）与四张由该 CA 签发的服务器证书：
// www/blog/api（SAN 各为对应 example.com 域名）与 wildcard（SAN *.example.com，
// 通配例）；另有 client-ca.crt 与其签发的 client.crt/client.key，用于下游 mTLS。
// 密钥每次运行随机生成；仓库提交的是运行产物，golden 快照只引用
...
func main() {
	if len(os.Args) == 2 && os.Args[1] == "-client-only" {
		generateClientIdentity()
		fmt.Println("testdata/certs: client CA + client identity generated")
		return
	}
	caKey, caCert := mustCA()
	...
	generateClientIdentity()
	fmt.Println("testdata/certs: CA + 4 server certificates + client identity generated")
}

func mustCA() (*ecdsa.PrivateKey, *x509.Certificate) {
	return mustNamedCA("esgw-test-ca", 1)
}

func mustNamedCA(commonName string, serial int64) (*ecdsa.PrivateKey, *x509.Certificate) {
...
func generateClientIdentity() {
	caKey, caCert := mustNamedCA("esgw-test-client-ca", 100)
	writeCert(caCert, nil, "client-ca")
	key, cert := mustClient(caCert, caKey)
	writeCert(cert, key, "client")
}

func mustClient(ca *x509.Certificate, caKey *ecdsa.PrivateKey) (*ecdsa.PrivateKey, *x509.Certificate) {
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	must(err)
	tpl := &x509.Certificate{
		SerialNumber: big.NewInt(101),
		Subject:      pkix.Name{CommonName: "esgw-test-client"},
		NotBefore:    time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
		NotAfter:     time.Date(2046, 1, 1, 0, 0, 0, 0, time.UTC),

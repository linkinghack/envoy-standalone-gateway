...
- 已完成：F5 static 保持 CA 文件 DataSource；xDS 提取 `ca/<listener>` validation-context Secret、写入 SourceMap 并生成 ADS/SDS 引用。P1 Secret 仍携带 filename DataSource，数据面必须落地同一路径材料；跨主机内联/独立证书材料传输明确留给 P2。

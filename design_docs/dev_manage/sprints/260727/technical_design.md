...
`Listener.tls.clientCA` 编译为 DownstreamTlsContext validation context 的 trusted CA，并设置 `require_client_certificate=true`。编译期解析 CA PEM，错误定位到 `spec.tls.clientCA`。static 产物保留文件 DataSource；xDS 产物提取为 `ca/<listener>` validation-context Secret 并通过 ADS/SDS 引用。当前 Secret 内容仍是 filename DataSource，因此数据面必须落地同一路径的证书材料；SDS 在 P1 只解决 Secret 身份、订阅和更新语义，跨主机内联/独立材料传输属于 P2。Upstream `maxConnections/maxPendingRequests` 必须位于 `1..2147483647`，默认 priority 为 DEFAULT；补齐 proto/golden/真实过载证据，不再只做“字段透传”。

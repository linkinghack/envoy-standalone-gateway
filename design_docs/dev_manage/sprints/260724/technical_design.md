...
- `internal/console/ui`：Vite 直接生成的生产构建产物，由 Go embed package 打包，不手工修改。
...
Vite `base=/` 生成 hash assets并在构建时清空、重建 `internal/console/ui`。Go `embed.FS` 只嵌入该目录；`index.html` 和 hash assets 由 Go 测试校验。开发环境由 Vite proxy `/api`、`/healthz`、`/readyz`、`/metrics` 到本机 esgw。

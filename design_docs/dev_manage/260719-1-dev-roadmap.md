...
| S3 | M1 | 配置域与发布流 | M-STORE（SQLite/迁移）；M-CONF（文件真源加载、Origin 定位 CRUD、版本快照、双视角 diff、回滚、发布流状态机、fsnotify 外部修改检测）；原生 static → IR 解析入口（FR-2.1） | [配置域](../system_design/260717-2-config-domain-design.md) | S1；第一阶段已完成（260721：M-STORE + 草稿哈希 + 快照）；发布流/API/fsnotify/原生 static 解析待后续 task |

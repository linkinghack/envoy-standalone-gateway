...
| S4 | M1 | 状态采集与生效确认 | M-STATE（admin client 白名单/串行调度、状态归一化、归属反解、确认快速通道、时间序列环形缓冲、Prometheus 透传）；发布流 CONFIRMING→EFFECTIVE 闭环联调 | [状态采集](../system_design/260717-4-state-collection-design.md) | S2、S3；已完成基础骨架（260722 T1~T3），S5 继续接入 API |

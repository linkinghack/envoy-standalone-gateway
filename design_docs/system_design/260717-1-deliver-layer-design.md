...
deliver:
  ...
  static:
    ...
    edsRebuildMinInterval: 30s   # static 模式下 EDS 通道节流（§3.3，DL5）
proc:
...
```go
// 在上一版 snapshot 基础上构造：仅 Endpoints 类型换新 version，
// 其余类型沿用原资源与原 version → SotW 按 version_info 比对，只向 Envoy 推 EDS。
edsVer := fmt.Sprintf("%s#eds-%d", ir.Version, seq)   // seq 进程内单调递增
snap.Resources[types.Endpoint] = cache.NewResources(edsVer, claItems)
```
...
static 形态下 EDS CLA 已内联进 Cluster（编译层 §1/F5 形态化），不存在文件级端点热更新通道（a 档未启用，§4.2），因此本通道在 static 模式**退化为**：更新内存副本 → `static.Render` 重渲染 → 原子写 → M-PROC hot restart。代价高（进程级替换），故：

- 节流：`deliver.static.edsRebuildMinInterval`（默认 30s，DL5），窗口内多次 `UpdateEndpoints` 合并为一次重渲染 + hot restart；
- 文档与控制台明示：**端点高频变动的场景（典型即 k8s）应选择 xds 模式**（矩阵 ①/②），static 模式下 `kubernetesService` 类 Upstream 的端点跟踪体验是降级的（FR-6.3 为 P2，该限制可接受）。
...
esgw (M-PROC)              envoy epoch N            envoy epoch N+1
    ...
    │                           │◄── unix socket 接管监听 fd ─┤  (同 base-id)
    │ 3. 轮询 M-STATE：/ready=LIVE 且 epoch=N+1（≤liveTimeout 30s）
    │ 4. SIGTERM ──────────────►│ DRAIN（≤drain-time-s）     │ serve (LIVE)
    │                           │ exit（异步 reap）          │
...
2. **成功**（步骤 4~5）：SIGTERM 旧 epoch → 旧进程按 `--drain-time-s` 排空存量连接后自行退出（envoy 保证），M-PROC 异步 reap，不阻塞 Apply。
3. **失败**（新进程退出码非零 / liveTimeout 超时）：SIGKILL 新 epoch，Apply 返回 `stage=hot_restart` 错误，附新进程 stderr 尾部（≤4KB，排障关键）。**安全不变量：`liveTimeout`(30s) << `--parent-shutdown-time-s`（配置下限 120s）**，保证回杀新 epoch 时旧 epoch 必然存活——旧进程在未收到 SIGTERM 且 parent shutdown 计时未到时持续服务，任何时刻至少一个 epoch 承接流量。
4. M-PROC 只在确认新 epoch live 后才触碰旧 epoch；判定期间 M-STATE 自身故障 → 按超时失败处理（安全方向，不误杀）。
...
| DL5 | static 模式 EDS 通道退化路径的 `edsRebuildMinInterval` 默认值（拟 30s）与是否提供总开关 | M1 联调期按实测定 |

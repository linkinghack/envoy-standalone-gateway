...
| U6 | **复用编译层确定性** | 编译产物 diff、变更摘要与产物导出基于编译层确定性渲染（[`260716-2-compile-ir-design.md`](260716-2-compile-ir-design.md) §5），UI 侧不做二次序列化，保证"看到的就是要下发的"；配置文本 diff 则直接对真源文件（[260717-2](260717-2-config-domain-design.md) §4.2），不经渲染、保留用户排版 |
...
| 乐观并发 | 草稿带**不透明字符串** `resourceVersion`，取值即配置域 `draftHash`（内容哈希，[260717-2](260717-2-config-domain-design.md) §2.2/§5.3——单调计数器覆盖不了 git pull 等外部文件修改，内容哈希天然覆盖）；对象写操作与 `publish` 可携带 `expectedResourceVersion`，不匹配返回 `409 CONFLICT` 由调用方重取——两个会话互相覆盖与"旧表单覆盖外部修改"被同一机制拦截 |
...
| 方法 | 路径 | 职责 |
|---|---|---|
| GET | `/api/v1/config/draft` | 读取草稿：`{sourceType, resourceVersion, updatedAt, files: [{path, content}]}`——`sourceType=protocol` 时为 `config.d/` 多文件原样返回，`native` 时单元素 `native.yaml`；保留用户文件组织与注释（[260717-2](260717-2-config-domain-design.md) §2.1/§3.2 的真源即多文件） |
| PUT | `/api/v1/config/draft` | 整体替换草稿文件集（同 `files` 形态，未列出的既有文件即删除；CLI/粘贴/专家模式工作流；`expectedResourceVersion` 可选） |
...
| GET | `/api/v1/config/versions/{id}/config` | 该版本配置文件集（`files` 形态，同草稿端点；`?format=tar` 打包下载） |
...
| POST | `/api/v1/config/versions/{id}/rollback` | 回滚：`{"publish": false, "force": false}`；`publish` 默认载入草稿、`true` 直接重发布（语义终审见 AD5）；草稿存在未发布修改时须 `force: true` 显式确认丢弃（[260717-2](260717-2-config-domain-design.md) §4.3） |
...
| GET | `/metrics` | esgw 管理面自身指标（`esgw_*`，M-CORE 注册表；**不含**数据面指标，边界见 [260717-4](260717-4-state-collection-design.md) §5.4） |
| GET | `/api/v1/envoy/stats/prometheus` | Envoy `/stats/prometheus` 原样流式透传（`envoy_*`，供外部长期采集）；须鉴权，机器凭据形态见 260717-4 §5.4/S4 |
...
// 200 响应
{
  ...
  "draftResourceVersion": "b58c22c19d0e"    // 不透明内容哈希（§3.1 乐观并发）
}
```
...
// GET /api/v1/config/status  响应（生效确认闭环，FR-4.5）
{
  ...
  "effective": {"irVersion": "3f9a1c2e8b01", "confirmedAt": "2026-07-17T08:30:02Z",
                "source": "config_dump"},
  "delivery": {"mode": "xds", "state": "confirmed"}
}
```

`delivery.state` 即下发层 `Status.Phase` 枚举（`idle|delivering|awaiting_confirm|confirmed|nacked|failed`，[`260717-1-deliver-layer-design.md`](260717-1-deliver-layer-design.md) §6.2，不另造别名）；effective 确认机制（config_dump 版本比对）归 [`260717-4-state-collection-design.md`](260717-4-state-collection-design.md)。前端发布流即轮询本端点直至 `confirmed` 或超时降级提示。
...
`format=unified`（默认）返回 `{"diff": "--- a/config.d/10-listeners.yaml\n+++ b/config.d/10-listeners.yaml\n..."}`——即 [260717-2](260717-2-config-domain-design.md) §4.2 的**配置文本视角**：对真源文件逐文件 unified diff（仅 CRLF/行尾空白规范化，保留用户排版）；**编译产物视角**的资源级 diff 经 `config/compiled` 与版本 `compiled` 端点对比获得（U6 的确定性保证作用于此）。
...
- **引导抢注风险与缓解**：无令牌的"先到先得"引导意味着——控制台若绑定非 loopback 地址且攻击者先于管理员访问，可抢注 admin。三层缓解：① 支持 `ESGW_INITIAL_ADMIN_PASSWORD` 环境变量，启动时直接创建 admin、跳过开放引导（容器/自动化场景推荐路径，S3 单条 `docker run` 即可注入）；② 引导窗口限时：启动后 30 分钟未完成引导则关闭 bootstrap 端点、需重启重开（Portainer 同款策略，消灭"长期暴露的未初始化实例"）；③ 绑定非 loopback 且处于引导态时，启动日志输出显式警告。控制台默认绑定地址的取舍（loopback vs 容器场景必需的 0.0.0.0）列 AD8。
...
| 决策点 | 选型 | 说明 |
|---|---|---|
| 框架 | **React + TypeScript + Vite**（D3/AD1 已终审） | 组件生态成熟，构建链路轻量且适合嵌入 Go 二进制 |
| 组件体系 | **shadcn/ui** | 采用可组合、可定制的源码组件，通过 design tokens 统一视觉规范 |
...
| 数据请求/状态 | **TanStack Query** | 管理服务端状态、缓存与请求生命周期；领域状态不放入 UI 组件层 |
...
兜底规则：**无法安全完成 AST 回写时，拒绝切换并停留在 YAML 视图提示用户**，绝不整文档重生成（那会丢注释与顺序）。AST 编辑库候选 eemeli/yaml 的 Document API，由控制台实现期验证后锁定。
...
| # | 事项 | 计划决策时机 |
|---|---|---|
| AD1 | **已终审**：React + TypeScript + Vite；shadcn/ui；TanStack Query；Monaco + monaco-yaml——即上游 D3 在本文范围的落地 | [M1 工程实施计划 §2](../dev_manage/engineering_plan/260722-1-m1-completion-plan.md#2-终审决策) |
...
| AD8 | 控制台默认监听地址：loopback 默认（裸机安全）与容器场景必需 0.0.0.0 的取舍；`ESGW_INITIAL_ADMIN_PASSWORD` 与引导窗口限时（§4.1）的默认参数 | M1 安装交付物设计时 |

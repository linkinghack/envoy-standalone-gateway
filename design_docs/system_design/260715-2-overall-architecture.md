...
| Web 前端 | **React + TypeScript + Vite**；组件体系采用 **shadcn/ui**；数据请求采用 TanStack Query；YAML 编辑器采用 Monaco + monaco-yaml | shadcn/ui 提供可组合、可定制的组件基础，并通过 design tokens 统一视觉规范；领域状态不放入 UI 组件层 | 定（D3/AD1 已终审，见 [M1 工程实施计划 §2](../dev_manage/engineering_plan/260722-1-m1-completion-plan.md#2-终审决策)） |
...
| D3 | **已终审**：React + TypeScript + Vite；shadcn/ui；TanStack Query；Monaco + monaco-yaml | [M1 工程实施计划 §2](../dev_manage/engineering_plan/260722-1-m1-completion-plan.md#2-终审决策) |
